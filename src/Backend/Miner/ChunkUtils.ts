import { LSMBucket, LSMLoc, LSMChunkData } from '../../_types/darkforest/api/ChunkStoreTypes';
...
export const toExploredChunk = (chunk: LSMChunkData) => {
  const planetLocs: WorldLocation[] = [];
...
export const getSiblingLocations = (chunkLoc: Rectangle) => {
  ...
  const ret: [Rectangle, Rectangle, Rectangle] = [siblingLocs[0], siblingLocs[1], siblingLocs[2]];
  return ret; // necessary for typescript type inference
};

export const getChunkOfSideLength = (coords: WorldCoords, sideLength: number) => {
...
export const addToChunkMap = (
  ...
  while (!maxChunkSize || sideLength < maxChunkSize) {
    ...
    const chunkFootprint = getChunkOfSideLength(chunkToAdd.chunkFootprint.bottomLeft, sideLength);
    chunkToAdd = {
      chunkFootprint,
      planetLocations,
